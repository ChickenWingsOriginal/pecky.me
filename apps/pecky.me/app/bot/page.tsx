...
export default function BotPage() {
  ...
  const checkBotStatus = async () => {
    ...
    try {
      const response = await fetch("https://rpc-mainnet.supra.com/rpc/v1/view", {
        method: "POST",
  ...
  const getFailureReason = async (txHash: string): Promise<string> => {
    try {
      const response = await fetch(`https://rpc-mainnet.supra.com/rpc/v1/transactions/${txHash}`);
      const data = await response.json();
  ...
  const handleActivateBot = async () => {
    ...
    try {
      ...
      if (isActive) {
        ...
        const failureMsg = await getFailureReason(result.transactionHash);
  ...
  return (
    <div className={css({ minH: "100vh", bg: "#fff3da", display: "flex", flexDir: "column", alignItems: "center", pb: "100px" })}>
      <div className={css({ maxW: "520px", w: "90%", mt: "40px", display: "flex", flexDir: "column", gap: "4px" })}>
        <RetroBox>
            <div className={css({ textAlign: "center", mb: "16px" })}>
              <div className={css({ mb: "12px" })}>
                <img src="/images/bot-icon.png" alt="PeckyBot" style={{ height: "62px", margin: "0 auto" }} />
              </div>
              <h1 className={css({ fontSize: "24px", fontWeight: "700", color: "#ff9000", mb: "12px" })}>
                PECKYBOT on Discord
              </h1>
            </div>

            <div className={css({ mb: "14px" })}>
              <div className={css({ fontSize: "14px", fontWeight: "600", color: "#2e2e2e", mb: "10px" })}>
                Get exclusive bot access:
              </div>
              <ul className={css({ fontSize: "13px", color: "#b48512", pl: "20px", lineHeight: "1.8", m: "0", mb: "12px" })}>
            ...
            <div className={css({ fontSize: "14px", color: "#a06500", mb: "8px" })}>
              Pay <span className={css({ fontWeight: "700" })}>one time 5,000 $SUPRA</span> to activate the bot.<br />
              After that, you only need Pecky as gas to keep the bot active.
            </div>
            <div className={css({ fontSize: "12px", color: "#ed7a00", fontWeight: "600" })}>
      ...
      <div className={css({ w: "100%", mt: "4px", px: "5%" })}>
        <RetroBox>
            <button
              ...
              disabled={isActivating || !state.isConnected}
              className={css({
                w: "100%",
                p: "14px 20px",
                bg: isActivating || !state.isConnected ? "#cccccc" : "linear-gradient(to right, #ffaa00, #ff7700)",
                color: "white",
                borderRadius: "12px",
                border: "none",
                fontSize: "15px",
                fontWeight: "600",
                cursor: isActivating || !state.isConnected ? "not-allowed" : "pointer",
                transition: "transform 0.1s",
                mb: "12px",
                _hover: isActivating || !state.isConnected ? {} : { transform: "scale(1.02)" },
              })}
            >
              {isActivating ? "Activating..." : "Activate PECKYBOT with Supra"}
            </button>
            <div className={css({ fontWeight: "600", color: "#ff9000", fontSize: "13px", mb: "16px" })}>
              Bot status: <span className={css({ color: "#2e2e2e" })}>{getBotStatus()}</span>
            </div>

            <hr style={{ margin: "12px 0", borderTop: "1.5px dashed #ffd36e", opacity: 0.6 }} />

            <div className={css({ mb: "14px", mt: "16px" })}>
              <label className={css({ fontSize: "13px", color: "#a06500", fontWeight: "600", display: "block", mb: "8px" })}>
              ...
              <div className={css({ display: "flex", gap: "10px", justifyContent: "center", alignItems: "stretch" })}>
                ...
                <button
                  ...
                  disabled={isExtending || !state.isConnected}
                  className={css({
                    flex: "1",
                    minW: "90px",
                    maxW: "150px",
                    ...
                    fontSize: "13px",
                    p: "0 12px",
                    bg: isExtending || !state.isConnected ? "#cccccc" : "linear-gradient(to right, #ffaa00, #ff7700)",
                    color: "white",
                    border: "none",
                    borderRadius: "11px",
                    fontWeight: "600",
                    cursor: isExtending || !state.isConnected ? "not-allowed" : "pointer",
                    transition: "transform 0.1s",
                    _hover: isExtending || !state.isConnected ? {} : { transform: "scale(1.02)" },
                  })}
                >
            ...
            <div className={css({ fontSize: "13px", color: "#ff9000", fontWeight: "500", textAlign: "center", mb: "8px" })}>
              {estimatedCost > 0 && `You will pay ${estimatedCost.toLocaleString()} Pecky`}
            </div>
            <div className={css({ fontSize: "15px", color: "#29cf41", fontWeight: "600", textAlign: "center", mb: "16px" })}>
              {botActive && `${daysLeft} days left active`}
            </div>

            <hr style={{ margin: "12px 0", borderTop: "1.5px dashed #ffd36e", opacity: 0.6 }} />

            <div className={css({ fontSize: "13px", color: "#888", textAlign: "center", lineHeight: "1.8", mt: "16px", mb: "16px" })}>
              <div className={css({ mb: "8px" })}>300,000 Pecky = 1 day bot activation.</div>
              <div className={css({ mb: "8px" })}>Use the bot as long as you keep extending!</div>
              <div>Need help? Ask in Discord!</div>
            </div>

            <hr style={{ margin: "12px 0", borderTop: "1.5px dashed #ffd36e", opacity: 0.6 }} />

            <div className={css({ display: "flex", justifyContent: "center", gap: "20px", alignItems: "center", mt: "16px" })}>
              <a
                ...
                title="Join Discord"
              >
                <img src="/images/Discord-pecky.png" alt="Discord" style={{ height: "42px" }} />
              </a>
              <a
              ...
              >
                <img src="/images/crystara.png" alt="Crystara" style={{ height: "36px" }} />
              </a>
            </div>
          </RetroBox>
        </div>
